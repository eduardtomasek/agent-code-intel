...
- Before editing an unfamiliar function/class/method, run
  ...
  HIGH/CRITICAL risk before proceeding.
...
and a running watcher rewrites that file from memory on every index —
...
def _preview_init(
    ...
    if write_docs:
        for name, enabled in (("CLAUDE.md", agent_target in ("claude", "both")), ("AGENTS.md", agent_target in ("codex", "both") or project.agents_md_project_owned(os.path.join(context.root, "AGENTS.md")))):
            if not enabled:
                continue
            state = project.doc_state(os.path.join(context.root, name))
            if state == "present" and not force_docs:
                plan("keep", "%s has a code-intel block" % name)
            elif state == "present":
...
def _apply_init(
    ...
    if write_docs:
        ...
        agents_path = os.path.join(context.root, "AGENTS.md")
        if agent_target in ("codex", "both") or project.agents_md_project_owned(agents_path):
            _write_doc(reporter, agents_path, force_docs)
    else:
        reporter.say("skipped (--no-docs)")
    reporter.say("")
